...
namespace WizardsNeverDie.Utility
{
    class ConvertUnits
    {
        ...
        public static Vector2[] ToDisplayUnits(Vector2[] simUnits)
        {
            for (int x = 0; x < simUnits.Length; ++x)
                simUnits[x] = ConvertUnits.ToDisplayUnits(simUnits[x]);
            return simUnits;
        }
        ...
        public static Vector2[] ToSimUnits(Vector2[] displayUnits)
        {
            for (int x = 0; x < displayUnits.Length; ++x)
                displayUnits[x] = ConvertUnits.ToSimUnits(displayUnits[x]);
            return displayUnits;
        }
